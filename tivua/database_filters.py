...
class FilterSQL:
    ...
    DEFAULT_SELECT_TABLE = "posts"

    def __init__(self, tables=None, sql=None, params=None):
        """
    ...
    @staticmethod
    def combine(op, a=None, b=None, is_unary=False):
        """
        ...
        a = FilterSQL() if a is None else a
        b = FilterSQL() if b is None else b
        ...
        return FilterSQL(tables=tables, sql=sql, params=params)


    def emit(self, select_keys, select_table=None, joins=None):
        """
        ...
        # Substitute "None" in the parameters with some defaults
        if joins is None:
            joins = FilterSQL.DEFAULT_JOINS
        if select_table is None:
            select_table = FilterSQL.DEFAULT_SELECT_TABLE

        # Generate short table aliases and substitute them into the SQL query
        tables = self.tables.copy()
        tables["$0"] = select_table
        ...
        select_table_alias = tables_subs["$0"]
        sql_prefix = "SELECT {} FROM {} AS {}".format(", ".join(
            map(lambda s: "{}.{}".format(select_table_alias, s), select_keys)),
            select_table, select_table_alias)

        # Generate the necessary JOINs
        sql_mid = ""
        for alias, table in tables.items():
            if alias == select_table_alias:
                continue
            sql_mid += (" JOIN {tbl_src} AS {src_alias} ON"
                        " ({src_alias}.{src} = {tar_alias}.{tar})").format(
                tbl_src=table, src_alias=alias, tar_alias=select_table_alias,
                src=joins[table], tar=joins[select_table])

        # If there are multiple JOINs on different tables, add a "GROUP BY"
        # statement
        if len(set(tables.values())) > 1:
            sql_suffix += " GROUP BY {}.{}".format(select_table_alias, joins[select_table])
        ...
        return sql, self.params, select_table_alias
...
class FilterLogical(Filter):
    ...
    def compile(self):
        ...
        if (isinstance(a, FilterFullText) and isinstance(b, FilterFullText)):
            return FilterFullText((a.match, op, b.match)).compile()
        else:
            # Generic logical operation
            return FilterSQL.combine(op, a.compile(), b.compile())
...
class FilterNot(Filter):
    ...
    def compile(self):
        return FilterSQL.combine("NOT", self.a.compile(), is_unary=True)
